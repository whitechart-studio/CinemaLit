...
def _log_audit(actor: str, action: str, details: str, project_id: str = DEFAULT_PROJECT_ID) -> None:
    ch_query(
...
def _load_scenes_from_clickhouse() -> List[Scene]:
    """Reconstructs Scene objects from the live ClickHouse tables (scenes +
    scene_cast + cast_members), so the unchanged crew logic can run against
    them exactly as it would against a locally-ingested script."""
    scenes_result = ch_query(
        "SELECT scene_id, scene_number, int_ext, location, time_of_day, page_count, description "
        "FROM cinemalit.scenes ORDER BY scene_number"
    )
    cast_result = ch_query(
        "SELECT sc.scene_id, cm.character_name FROM cinemalit.scene_cast sc "
        "JOIN cinemalit.cast_members cm ON sc.cast_id = cm.cast_id"
    )
    characters_by_scene = {}
...
def _build_state_from_clickhouse(project_id: str = DEFAULT_PROJECT_ID) -> ProjectState:
    state = ProjectState(project_id=project_id, name="CinemaLit Live Project")
    state.scenes = _load_scenes_from_clickhouse()
    return state
...
def analyze_script(script_text: str) -> dict:
    ...
    max_id_result = ch_query("SELECT MAX(scene_id) FROM cinemalit.scenes")
    next_id = int(max_id_result.get("data", [[0]])[0][0] or 0) + 1

    for idx, s in enumerate(scenes):
        ch_query(
            "INSERT INTO cinemalit.scenes "
            "(scene_id, scene_number, int_ext, location, time_of_day, page_count, shoot_day, status, description) "
            "VALUES ({sid:UInt32}, {sn:String}, {ie:String}, {loc:String}, {tod:String}, {pc:Float32}, {sd:UInt8}, {st:String}, {desc:String})",
            {
                "sid": next_id + idx,
                "sn": s.number,
                "ie": s.setting,
    ...
    _log_audit("DIRECTOR_AGENT", "ANALYZE_SCRIPT", f"Wrote {len(scenes)} scenes")
...
def list_scenes() -> dict:
    ...
    state = _build_state_from_clickhouse()
...
def estimate_budget_pressure(target_budget: float = 5000.0) -> dict:
...
def suggest_budget_savings(target_budget: float = 5000.0) -> dict:
...
def generate_schedule(target_days: int = 2) -> dict:
...
def create_production_tasks() -> dict:
...
def request_gate_approval(gate_id: str, gate_name: str, rationale: str = "Approved by Director") -> dict:
    ...
    ch_query(
        ...
        {"p": DEFAULT_PROJECT_ID, "g": gate_id},
    )
    ch_query(
        ...
        {"p": DEFAULT_PROJECT_ID, "g": gate_id, "n": gate_name, "r": rationale},
    )
    _log_audit("DIRECTOR_AGENT", "APPROVE_GATE", f"{gate_id}: {rationale}")
