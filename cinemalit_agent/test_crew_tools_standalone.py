...
def main() -> int:
    ...
    scenes_result = list_scenes(PROJECT_ID)
    all_ok &= check(
    ...
    risks_result = find_risks(PROJECT_ID)
    all_ok &= check(
    ...
    breakdown_result = production_breakdown(PROJECT_ID)
    all_ok &= check(
    ...
    budget_result = estimate_budget_pressure(PROJECT_ID, target_budget=5000.0)
    all_ok &= check(
    ...
    # --- suggest_budget_savings ---
    savings_result = suggest_budget_savings(PROJECT_ID, target_budget=100.0)  # force OVER_BUDGET to guarantee proposals
    all_ok &= check(
    ...
    # --- generate_schedule ---
    schedule_result = generate_schedule(PROJECT_ID, target_days=2)
    all_ok &= check(
    ...
    # --- create_production_tasks ---
    tasks_result = create_production_tasks(PROJECT_ID)
    all_ok &= check(
    ...
    before_count = ch_query(
        "SELECT count() FROM cinemalit.scenes WHERE project_id = {p:String}", {"p": PROJECT_ID}
    ).get("data", [[0]])[0][0]
    analyze_result = analyze_script(TEST_SCRIPT, PROJECT_ID)
    ...
    approve_result = request_gate_approval(TEST_GATE_ID, "Test Gate", PROJECT_ID, "Verifying standalone")
    all_ok &= check(
    ...
    audit_result = get_audit_log(PROJECT_ID)
    all_ok &= check(
    ...
    mem_budget = query_studio_memory("what is our budget status", PROJECT_ID)
    mem_risk = query_studio_memory("any risks I should know about", PROJECT_ID)
    mem_default = query_studio_memory("tell me about the scenes", PROJECT_ID)
    all_ok &= check(
