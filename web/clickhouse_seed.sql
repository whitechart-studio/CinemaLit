-- ============================================================
-- CinemaLit Studio — ClickHouse Production Database Seed
-- Film: "Shadows of Midnight" (Demo Project)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS cinemalit.scenes (
    ...
    location        String,
    time_of_day     LowCardinality(String),   -- DAY / NIGHT / DAWN / DUSK
    page_count      Float32,
    shoot_day       UInt8,
    status          LowCardinality(String),   -- scheduled / locked / complete / vfx_required
    description     String,
...
CREATE TABLE IF NOT EXISTS cinemalit.cast_members (
    ...
    actor_name      String,
    role_type       LowCardinality(String),   -- lead / supporting / day_player / extra
...
-- Scene-Cast crosswalk (which actors appear in which scenes)
CREATE TABLE IF NOT EXISTS cinemalit.scene_cast (
    scene_id        UInt32,
    cast_id         UInt32,
    status          LowCardinality(String)    -- W / SW / H / WF
) ENGINE = MergeTree()
...
CREATE TABLE IF NOT EXISTS cinemalit.budget_items (
    item_id         UInt32,
    category        LowCardinality(String),   -- Above-the-Line / Production / Post / VFX / Marketing
    sub_category    String,
    description     String,
...
CREATE TABLE IF NOT EXISTS cinemalit.elements (
    element_id      UInt32,
    element_type    LowCardinality(String),   -- prop / wardrobe / vfx / sfx / vehicle / set
    name            String,
    description     String,
    scene_ids       Array(UInt32),
    cost_usd        Float64,
    vendor          String,
    status          LowCardinality(String),   -- planned / confirmed / booked / wrapped
...
CREATE TABLE IF NOT EXISTS cinemalit.shots (
    ...
    lens_mm         UInt16,
    movement        String,
    framing         LowCardinality(String),   -- WS / MS / CU / ECU / OTS / POV
    description     String,
    status          LowCardinality(String),   -- planned / setup / complete
    PRIMARY KEY (shot_id)
) ENGINE = MergeTree()
ORDER BY shot_id;
